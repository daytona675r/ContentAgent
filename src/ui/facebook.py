...
def facebook_tab():
    ...
    if hasattr(final_state, "aggregated_scores"):
        agg_scores = final_state.aggregated_scores.get("facebook", None)
    ...
    with st.container(height=200):
        if hasattr(final_state, "final_content"):
            content = final_state.final_content.get("facebook", {})
        elif isinstance(final_state, dict) and "final_content" in final_state:
            content = final_state["final_content"].get("facebook", {})
        else:
            content = {}
        
        post = content.get("post", "No Facebook content available.")
        hastags = content.get("hashtags", "No Fcebook hastags available.")
        st.markdown(post)
        st.markdown(f"**{hastags}**")

    # Show refinement history
    if hasattr(final_state, "refinement_history"):
        refinements = final_state.refinement_history.get("facebook", [])
