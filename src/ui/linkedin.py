...
def linkedin_tab():
    ...
    if hasattr(final_state, "aggregated_scores"):
        agg_scores = final_state.aggregated_scores.get("linkedin", None)
    ...
    with st.container(height=200):
        if hasattr(final_state, "final_content"):
            content = final_state.final_content.get("linkedin", {})
        elif isinstance(final_state, dict) and "final_content" in final_state:
            content = final_state["final_content"].get("linkedin", {})
        else:
            content = {}
        
        post = content.get("post", "No LinkedIn content available.")
        hastags = content.get("hashtags", "No LinkedIn hastags available.")
        st.markdown(post)
        st.markdown(f"**{hastags}**")

    # Show refinement history
    if hasattr(final_state, "refinement_history"):
        refinements = final_state.refinement_history.get("linkedin", [])
